...
@require_http_methods(["GET"])
def ajax_edit_content_html(request,event_type_id=0):
    if(event_type_id == 0):
        return HttpResponse("传入参数异常")
    event_type = EventType.objects.get(id=event_type_id)
    return render_to_response("ajax/ajaxEditPageEventContent.html", {"id": event_type_id,"eventTypeForm": AddEventTypeForm(instance = event_type)})

@login_required(login_url='/backoffice/login')
@require_http_methods(["POST"])
def event_type_update(request):
    try:
        form = AddEventTypeForm(request.POST, request.FILES, instance=EventType.objects.get(id=request.POST["id"]))
        if form.is_valid():
            form.save()
            result = u'更新事件类型成功'
        else:
